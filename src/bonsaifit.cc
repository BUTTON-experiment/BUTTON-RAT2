...
// *************************************************************
inline void bonsaifit::fiducial_dev(float &devx, float &devy, float &devz,
                                    float &dev) {
...
// *************************************************************
inline void
bonsaifit::end_search(float *point, int indx, int indy, int indz,
  ...
  printf("-> (%8.2f,%8.2f,%8.2f,%6.1f) q=%8.6f radius=%6.2f<%6.2f\n", point[0],
         point[1], point[2], point[STORERES], point[STOREQ], point[STORERADIUS],
         rmax);
#endif
  return;
}
...
void bonsaifit::search(float *start, float rmax) {
...
#ifdef DEBUG
  printf("search %6.2fcm (%8.2f,%8.2f,%8.2f,%6.1f)", rad, start[0], start[1],
         start[2], start[STORERES]);
#ifdef DEBUG_TWO
  printf("\n");
#endif
#endif
  while (1) {
...
#ifdef DEBUG_TWO
      printf("max outside: (%8.2f,%8.2f,%8.2f)", vertex[3 * max],
             vertex[3 * max + 1], vertex[3 * max + 2]);
#endif
    ...
    if (max != 0) {
      // if maximum is not in the dodecahedran center, move center to the
      // maximum
