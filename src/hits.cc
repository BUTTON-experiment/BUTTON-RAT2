...
// **********************************************
inline void hits::force_zero(double *mat, int *rowind, int *colind, int pass,
                             int ind) {
...
int hits::inv35(double *mat, int *rowind, int *colind) {
  ...
  for (pass = 0; pass < 3; pass++) {
    ...
    if (factor == 0)
      return (pass); // if zero already, break off inversion
...
void hits::qsort(short int *list, short int n, short int *first) {
  if (n < 2)
    return;   // no need to sort for 1 or zero hits
  if (n == 2) // 2 hits are easy to sort ...
  {
    if (times[*list] < times[list[1]])
      return;
    n = list[1];
    list[1] = *list;
    *list = n;
  ...
  for (firstp = secondp = 0;
    ...
    if (times[firstel] <
        times[second[secondp]]) {       // take element of the first sublist
...
// positions
// **********************************************
hits::hits(int ns, float *set, float *pmt_loc, int *bad_ch, int n_raw,
           int *cable, float *tim_raw, float *chg_raw) {
...
hits::hits(int ns, float *set, pmt_geometry *geom, int nh, int *cab, float *t,
           float *q) {
  int i, j, number, pos, *nh2;
...
// find vertex from four-hit combination
// **********************************************
int hits::vertex4(int *fourcombo, int *hitlist, int listsize, int &hbegin,
                  int &hend, float tsig, float cyl_radius, float cyl_height,
                  float dwallmin, double *vert, float *gdn) {
  ...
  for (s = 0; s < ns; s++) {
    ...
    if ((tmin = cyl_radius - rad) < dwallmin)
      continue;
    height = fabs(vert[4 * s + 2]);
    if ((tmax = cyl_height - height) < dwallmin)
      continue;
    if (tmax < tmin)
      tmin = tmax;
    ...
    vertex[1] = vert[4 * s + 1];
    vertex[2] = vert[4 * s + 2];
    /*printf("%3d %3d %3d %3d (1):
    ",fourcombo[0],fourcombo[1],fourcombo[2],fourcombo[3]); printf("%8.2lf
    %8.2lf %8.2lf %6.1lf ",vert[4*s],vert[4*s+1],vert[4*s+2],vert[4*s+3]);
    printf("%8.2f",tmin);*/
    tmin = vert[4 * s + 3] + tmin * CM_TO_NS - 3 * tsig;
    rad += cyl_radius;
    height += cyl_height;
    tmax = vert[4 * s + 3] + sqrt(rad * rad + height * height) * CM_TO_NS +
           3 * tsig;
    // printf("(%6.1f) to %8.2f(%6.1f) ",tmin,sqrt(rad*rad+height*height),tmax);
    gdn[s] = 4;
    while ((hbegin > 0) && (times[hitlist[hbegin]] >= tmin))
      hbegin--;
    while ((hbegin < listsize - 1) && (times[hitlist[hbegin]] < tmin))
      hbegin++;
    while ((hend < listsize - 1) && (times[hitlist[hend]] <= tmax))
      hend++;
    while ((hend > 0) && (times[hitlist[hend]] > tmax))
      hend--;
    // printf("begin %6.1f end %6.1f
    // ",times[hitlist[hbegin]],times[hitlist[hend]]);
