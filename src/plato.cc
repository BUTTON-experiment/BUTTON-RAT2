...
void dodecahedron::interpolate(float *values, float *inter) {
  ...
  // chi^2 minimization
  float axx = 0.125 * (-vap1 + 3 * vap2 +
                       0.5 * (vap3 + vap4 + vap5 + vap6 -
                              rt5 * (vap3 - vap4 - vap5 + vap6))) -
              values[0];
  float axy = 0.125 * rt5 * (rsm * (vap3 - vap6) - rsp * (vap4 - vap5));
  float axz =
      0.25 * vap2 + 0.125 * rt5 * (sm * (vap3 + vap6) - sp * (vap4 + vap5));
  float ayy = 0.125 * (-vap1 - vap2 + 1.5 * (vap3 + vap4 + vap5 + vap6) +
                       0.5 * rt5 * (vap3 - vap4 - vap5 + vap6)) -
              values[0];
  float ayz = 0.125 * rt5 * (rsp * (vap3 - vap6) + rsm * (vap4 - vap5));
  float azz = 0.5 * vap1 - values[0];
  float bx =
      -0.25 * irt5 * vam2 - 0.125 * (sm * (vam3 + vam6) - sp * (vam4 + vam5));
  ...
  inter[1] = by - ayz * bz;
  inter[3] =
      sqrt(inter[0] * inter[0] + inter[1] * inter[1] + inter[2] * inter[2]);
  if (inter[3] > 1.5) {
    inter[3] = 1.5 / inter[3];
    inter[0] *= inter[3];
    inter[1] *= inter[3];
...
icosahedron::icosahedron(void) {
  ...
  dode.get_vectors(dodevec, // rotate dodecahedron
                   sinthdo * sqrt(con2 / 3) / con1, sinthdo / (sqrt(3) * con1),
                   sqrt(con2 / 3));
