  ...
  */
  ...
  void VertexGen_PhotonRay::SetState(G4String newValues)
  {
    ...
    if (is.fail()){
      ...
      if (is.fail())
	      RAT::Log::Die("VertexGen_PhotonBeam: Incorrect vertex setting " + newValues);
      fMaterial = material;

      // get the scintillation wavelength spectrum
      RAT::DBLinkPtr loptics = RAT::DB::Get()->GetLink("OPTICS", material);
      ...
      for (unsigned i=1; i<wlarr.size(); i++)
	      if(wlarr[i-1] >= wlarr[i])
	        RAT::Log::Die("VertexGen_PhotonBeam: wavelengths out of order");
      ...
      RAT::LinearInterp<double> energyInterp(wlarr, wlamp);
      double step = 1.0e9;
    ...
    is >> exp;
    if (exp < 0.0)
      RAT::Log::Die("VertexGen_PhotonBeam: Exponential time constant must be positive");
