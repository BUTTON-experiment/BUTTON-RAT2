...
BonsaiProc::BonsaiProc() : RAT::Processor("bonsai") {
  RAT::DB *db = RAT::DB::Get();
  printf("BonsaiProc: Initialisation of the BONSAI processor.\n");
}

void BonsaiProc::BeginOfRun(RAT::DS::Run *run) {
  ...
  printf("BonsaiProc: Loading useCherenkovAngle -> %d\n", bs_useCherenkovAngle);
  printf("BonsaiProc: Loading nTmin -> %3.1f\n", bs_nTmin);
  printf("BonsaiProc: Loading nTmax -> %3.1f\n", bs_nTmax);
  printf("BonsaiProc: Resulting in a n%d ns time window\n",
         int(bs_nTmax - bs_nTmin));
  printf("BonsaiProc: Loading Group Media Light Speed -> %3.1f ns/cm\n",
         hits::NS_TO_CM);
  printf("BonsaiProc: User requested Likelihood filename -> %s \n",
         bs_likeFilename.c_str());

  bs_pmtinfo = run->GetPMTInfo();
  // printf("BonsaiProc: Was able to load in PMTInfo.\n");
  // printf("BonsaiProc: Initialization of Geometry object\n");
  bs_inpmt = run->GetPMTInfo()->GetPMTCount();
  // printf("BonsaiProc: Number of PMT counts:%d\n",bs_inpmt);
  ...
  // printf("BonsaiProc: PMT boundary (r,z):(%4.1f cm, %4.1f
  // cm)\n",bsgeom->cylinder_radius(),bsgeom->cylinder_height());
  // printf("BonsaiProc: Initialisation of Likelihood object.\n");
  bslike = new likelihood(bsgeom->cylinder_radius(), bsgeom->cylinder_height());
  bsfit = new bonsaifit(bslike);
}

int BonsaiProc::nwin(float twin, float *v, int nfit, int *cfit, float *tfit,
                     int *cwin) {
  if (nfit <= 0)
    return (0);
  ...
  for (hit = 0; hit < nfit; hit++) {
    ...
    dz = pos.Z() * 0.1 - v[2];
    tsort[hit] = ttof[hit] =
        tfit[hit] - sqrt(dx * dx + dy * dy + dz * dz) * hits::CM_TO_NS;
  }
  ...
  for (hit = 0; hit < nfit; hit++) {
    if (ttof[hit] < tsort[hstart])
      continue;
    if (ttof[hit] > tsort[hstop])
      continue;
    cwin[nwin++] = cfit[hit];
  }
  if (nwin != nwindow)
    printf("nwin error %d!=%d\n", nwin, nwindow);
  return (nwindow);
}

RAT::Processor::Result BonsaiProc::Event(RAT::DS::Root *ds, RAT::DS::EV *ev) {
  ...
  bsgdn = new goodness(bslike->sets(), bslike->chargebins(), bsgeom, bs_nhit,
                       bs_cables, bs_times, bs_charges);
  bs_nsel = bsgdn->nselected();

  // printf("BonsaiProc:Number of Selected Tubes:%d/%d\n", bs_nsel,bs_nhit);
  ...
  if (bs_nsel < 4) {
    ...
    if (bs_useCherenkovAngle == 0)
      use_cherenkov_angle = false;
    bslike->maximize(bsfit, bsgrid,
                     use_cherenkov_angle); // use_cherenkov_angle);
    ...
    *bs_bonsai_vtxfit = bsfit->xfit();
    ...
    TVector3 fitPosition(10. * bsfit->xfit(), 10. * bsfit->yfit(),
                         10. * bsfit->zfit());
    //     TVector3 fitDirection(1.0, 0.0, 0.0);
    double fitEnergy = -1.0;
    ...
    // Do direction fit
    int nTwin = nwin(float(bs_nTmax - bs_nTmin), bs_bonsai_vtxfit, bs_nhit,
                     bs_cables, bs_times, bs_cables_win);
    // int
    // nTwin=nwin(9,bs_bonsai_vtxfit,bs_nhit,bs_cables,bs_times,bs_cables_win);
