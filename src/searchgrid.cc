...
// *************************************************************
inline double searchgrid::dis2(double &dx, double &dy, double &dz, int p1,
                               int p2) {
...
// *************************************************************
inline int searchgrid::close_to(double &dx, double &dy, double &dz, int p1,
                                int p2min, int p2max, double d2max) {
  // loop through point p2 from p2min to p2max
  for (; p2min < p2max; p2min++)
    // stop, if distance is closer than limit
    if (dis2(dx, dy, dz, p1, p2min) < d2max)
      return (p2min);
  return (-1);
}
...
void searchgrid::sparsify(float dmin) {
  ...
  for (point = start + 1; point < stop;
       point++) { // find another point close-by?
    sec = close_to(dx, dy, dz, point, stop, npoint, dmin);
    if (sec == -1) { // no, then copy the pont
      if (npoint >= mpoint)
        expand_size(stop - point);
