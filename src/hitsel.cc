...
short int hitsel::make_causal_table(short int *&related, short int *&relations,
                                    float twin, float resolution,
                                    float tcoincidence) {
  ...
  for (row = 0, relp = related; row < nsel; row++)
    for (column = row + 1; column < nsel; column++)
      if (((*relp++) = 2 * causal(selected[row], selected[column], twin,
                                  resolution, tcoincidence))) {
        nr_rel[row]++;
        nr_rel[column]++;
      }
  /*-------------------------------------------------------------------
    ...
    if ((curnrel = nr_rel[row]) >= MINHIT) {
      ...
      if (n_cl == 0)
        ...
        if ((curnrel > *relations) ||
            ((curnrel == *relations) && (hitcharge(pmt) > hitcharge(*select))))
          pmt2 = 0;
        else {
  ...
  nsel = n_cl + 2;
  for (row = 0; row < n_cl; row++)
    for (relp += column = row + 1, relp1 = relations + nsel * row;
         column < n_cl; column++)
      if (*relp++) {
  ...
  for (row = 0; row < n_cl; row++)
    for (column = 0, relp = relations + nsel * row + 2, relp[-1] = relp[-2];
         column < relp[-2]; column++)
      if (relp[column] > row) {
        relp[-1] = column;
        break;
      }
  ...
  //     printf("
  //     %d",causal(selected[row],selected[column],twin,resolution,tcoincidence));
  return (n_cl);
}
...
/*-------------------------------------------------------------------
  ...
  -------------------------------------------------------------------*/
short int hitsel::reduce(int size, short int *source, short int *destination,
                         short int *occurence) {
  int nr, i, j;
  short int copy;
...
// **********************************************
void hitsel::select(float dlim, float tlim, float twin, float resolution,
                    float tcoincidence) {
  int s, i;
  short int n;
  ...
  do {
    ...
    s--;
  } while (((n = clus_sel(dlim, tlim, twin, resolution, tcoincidence)) <= 0) &&
           (s >= 0));
  if (n <= 0) {
    nsel = ntot();
    selected = new short int[nsel];
...
// -------------------------------------------------------------------
void hitsel::pmtcentroid(int n, float *ctrd, float *axes, float rmax,
                         float zmax) {
  double fac;
  int i;
...
short int hitsel::mrclean(float dislimit, float tlimit) {
  ...
  for (row = 0; row < nsel; row++)
    for (column = 0; (column < nsel) && (!is_selected[row]); column++)
      if (clean(selected[row], selected[column], dislimit2,
                tlimit)) { // select the non-isolated hit
        is_selected[row] = 1;
  ...
  -------------------------------------------------------------------*/
{
  ...
  for (ntest = n_gd, row = 0; row < ntest; row++)
    for (column = row + 1, relp += column,
        relp1 = relations + (n_gd + 2) * row + 2;
         column < n_gd; column++)
      if (*relp++) {
        ...
        *(clus) = row;
        ...
        for (new_pmt = 2; new_pmt < clus_size; new_pmt++) {
          ...
          for (pmt1_index = new_pmt + 1; /* lists are identical so far */
               ...
               (clus[pmt1_index] == relp2[pmt2_index]);
               pmt1_index++)
            pmt2_index++;
        ...
        if (clus_size >= min_size) {
          ...
          for(i=0; i<clus_size; i++,printf("\n"))
            for(j=0; j<clus_size; j++)
              printf("
          %d",causal(temp[i],temp[j],twin,resolution,tcoincidence));
          ...
          if (clus_size > max_size) {
            ...
            while (relations[(ntest - 1) * (n_gd + 2)] + 1 < min_size)
              ntest--;
          }
          clus += clus_size;
          n_clus++;
