...
binfile::binfile(char *name, char mode) {
  ...
  char *default_name =
      strcat(getenv("BUTTONDATA"), BUTTON::BonsaiProc::bs_likeFilename.c_str());
  if (fp == NULL) {
    printf("BonsaiProc: Legacy mode -> Unable to a local file named %s! "
           "Loading user requested likelihood instead.\n",
           name);
    printf("BonsaiProc: Loading in likelihood file named -> %s \n",
           default_name);
