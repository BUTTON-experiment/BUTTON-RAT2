...
void goodness::qsort(short int *list, short int n) {
  ...
  for (firstp = secondp = 0;
    ...
    if (ttof[firstel] <
        ttof[second[secondp]]) {        // take element of the first sublist
...
void goodness::bubble(void) {
  ...
  if (sort_list[0] == -1) {
    for (loop = 0; loop < nselected(); loop++)
      sort_list[loop] = loop;
    qsort(sort_list, nselected());
    return;
  }

  for (loop = begin = 0, end = nselected() - 1; loop < nselected() - 1;
       loop++) {
    ...
    if (swap == -1)
      return; // if no disorder was found, stop
...
float goodness::quality(float *vertex) {
  ...
  if (nselected() < 2)
    return (-1); // can't calculate with less than two hits
  ...
  for (tcent = start = 0; start < nselected(); start++) {
    nwin = sel(start);
    tcent +=
        (ttof[start] = hittime(nwin) - tof(vertex, hitdir + 3 * start, nwin));
  }
  tcent /= nselected();
  ...
  while (1) { // calculate goodness for all hits inside the window, if
    ...
    if (++nwin > gdns) {
      ...
      if ((deviation = tau_end - 1 - tav) >
          0) { // if tav is below minimum time (tau_end-1), use minimum time
      ...
      } else if ((deviation = tav - tau_begin - 1) >
                 0) { // if tav is above maximum time (tau_begin+1), use max.
                      // time
        if ((g = nwin * (1 + tav * tav - deviation * deviation) - tau2_sum) >
            gdns) { // if goodness is largerest so far, set t0 and gdns
          tau0 = tau_begin + 1;
    ...
    if (start + nwin >=
        nselected()) { // if the the end of the window is the end of
