...
void ariadne(float *vertex, int *nrhits, float *position, float *direction,
             float *goodness, float *quality, float *cos_scat, int *nrscat);
void cone_intersect(float *axis1, float *axis2, float *dir1, float *dir2,
                    int &solution);
void find_largest_cluster(char *active, float *directions, int nrdir,
                          float *clusdir, float &limit, int &max_index,
...
void final_scan(char *active, float *directions, int nrdir, float *clusdir,
                float limit, int &max_index, float &maxmag, int &nradd,
                int &clus_index);
void sum_dir(float *directions, char *active, int nrdir, float *sum, int &nradd,
             float *direc, float cos_min);
  ...
  for (row = 0; row < *nrhits; row++)
    for (column = row + 1; column < *nrhits; column++) {
      cone_intersect(vector + 3 * row, vector + 3 * column,
                     directions + 4 * nrdir, directions + 4 * (nrdir + 1), sol);
      if (sol == 2) {
...
#define COS_THETA 0.719
  ...
  sin_alpha2 = (ax1[0] * ax1[0] + ax1[1] * ax1[1] + ax1[2] * ax1[2]) *
                   (ax2[0] * ax2[0] + ax2[1] * ax2[1] + ax2[2] * ax2[2]) -
               cos_alpha * cos_alpha;
  if (sin_alpha2 == 0) {
    solution = 3;
    return;
...
void find_largest_cluster(char *active, float *directions, int nrdir,
  ...
  for (row = 0; row < nrdir; row++)
    if (active[row]) {
      ...
      if (clusdir[clus_index * 4 + 3] < limit)
        continue;
...
void refine_cluster(char *active, float *directions, int nrdir, float *clusdir,
  ...
  maxmag = 1.5;
  for (i = 0; i < 3; i++)
    tempdir[i] = clusdir[4 * (max_index - 1) + i];
  *cos_scat = 1;
  *nrscat = 0;
  max_index = -1;
  for (i = 0; i < clus_index; i++)
    if (clusdir[3 + 4 * i] > limit) {
      ...
      sum_dir(directions, active, nrdir, tempdir, nradd, clusdir + 4 * i,
              COS_CUT);
 ...
 ----------------------------------------------------------------------*/
{
  ...
  for (index = 0; index < nrdir; index++)
    if (direc[0] * directions[4 * index] +
            direc[1] * directions[4 * index + 1] +
            direc[2] * directions[4 * index + 2] >
        cos_min) {
      active[index] = 0;
      for (j = 0; j < 3; j++)
        sum[j] += directions[4 * index + 3] * directions[4 * index + j];
      ++nradd;
    }
