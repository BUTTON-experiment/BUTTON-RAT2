...
void timefit::integrate_dist(void) {
  ...
  for (set = 0; set < (int)nlike - 1; set++)
    printf("BonsaiProc: Loaded likelhood info -> pdf for %5.2fpe<=q<%5.2fpe: "
           "zero at %d total=%fns\n",
           qminmax[set], qminmax[set + 1], nneg[set],
           0.2 * TBIN * pdf_sum[4 * part + offset[set] - 1]);
  printf("BonsaiProc: Loaded likelhood info -> pdf for %5.2fpe<=q: zero at %d "
         "total=%fns\n",
...
void timefit::load_dist(char *fname) {
  ...
  // ********************** load pdfs from file **********************
  // printf("BonsaiProc: Loading pdfs...(%s
  // %s)\n",fname,BUTTON::BonsaiProc::bs_likeFilename.c_str());
  binfile bf(fname, 'r');
  ...
  if (bgfit)
    printf("BonsaiProc: Turn on background fitting\n");
  ...
  sx4 = nbin * (nbin * nbin * nbin + (2 * nbin * nbin * nbin * nbin + 3) / 5) +
...
inline float timefit::histo_t0(void) {
  ...
  for (lp = time_histo, i = 0; i < NHIST; i++)
    *(lp++) = 0;
  ...
  for (max = 0, i = 1; i < NHIST; i++)
    if (time_histo[i] > time_histo[max])
      max = i;
...
inline float timefit::maxloglike(int max) {
  ...
  diff2 = diff2 * diff2;
  interpol =
      time_histo[max] + 0.125 * diff2 / (1.6 * time_histo[max] - 0.8 * sum);
...
// **********************************************
inline void timefit::sigrange(float tofmin, float tofmax, float *norm_fact,
                              float *time_range) {
...
float timefit::makelike(float t0) {
  ...
  for (nbg = set = offs = i = 0; set < (int)nlike; offs = offset[set++]) {
    ...
    if ((tofmax - tofmin - time_range[set]) > 0)
      sig[set] = event_hits->nhits(set) -
                 nbg * (tofmax - tofmin) / (tofmax - tofmin - time_range[set]);
    else
      return (-1e10);
...
inline float timefit::makelike(int set, float &bg, float range) {
  ...
  if ((sig[set] < 0) || (sig[set] > event_hits->nhits(set)))
    return (-1e10);
  bg = (event_hits->nhits(set) - sig[set]) / range;
  nbg = 0;
  for (llmax = 0, i = event_hits->first_hit(set);
       i < event_hits->beyond_last_hit(set); i++)
    if (like[i] == 0.0)
      nbg++;
    else
...
// **********************************************
inline void timefit::fitdir(float *vertex, float &dt, centroid *&centr,
                            float &mucang) {
  ...
  centr->centre(direct);
  direct[3] =
      sqrt(*direct * (*direct) + direct[1] * direct[1] + direct[2] * direct[2]);
  if (direct[3] > 0) {
    *direct /= direct[3];
    direct[1] /= direct[3];
    direct[2] /= direct[3];
  ...
  for (set = 0; ((set < (int)nlike) && (event_hits->sumhits(set) > 30));
       offs = offset[set++])
    if (qminmax[set] >= 9)
      break;
  ...
  for (ind = event_hits->first_hit(set); set < (int)nlike; set++)
    for (; ind < event_hits->beyond_last_hit(set); ind++) {
      bin = (int)(200 *
                  (*direct * dir[3 * ind] + direct[1] * dir[3 * ind + 1] +
                   direct[2] * dir[3 * ind + 2]) /
                  (direct[3]));
      if (bin < 0)
        bin = 0;
      if (bin > 200)
...
void timefit::fittime(int ndt, float *dts, float &tpeak, float &dt) {
  ...
  for (lp = time_histo, i = 0; i < NTIME; i++)
    *(lp++) = 0;
  for (i = 0; i < ndt; i++) {
...
// **********************************************
float timefit::fittime(short int fast, float *vertex, float *direct,
                       float &dt) {
  int maxprod;
  axes ax;
  centroid *centr;
  ...
  if (centr->trace() < 0.001) {
    ...
    if (maxprod == 0) {
      ...
      if (direct[4] > 0)
        direct[4] = fabs(centr->yy() - centr->zz()) / direct[4];
    } else if (maxprod == 1) {
      direct[4] = (centr->xx() + centr->zz());
      if (direct[4] > 0)
        direct[4] = fabs(centr->xx() - centr->zz()) / direct[4];
    } else if (maxprod == 2) {
      direct[4] = (centr->xx() + centr->yy());
      if (direct[4] > 0)
        direct[4] = fabs(centr->xx() - centr->yy()) / direct[4];
    }
  }
  delete (centr);
  if (bgfit)
...
// **********************************************
float timefit::fittime(float *vertex, float *direct, float &dt, float tmi,
                       float tpl) {
  int maxprod;
  axes ax;
  centroid *centr;
...
inline void timefit::corfunc(int *&bin, float *&cor, double *direct,
                             double cang) {
  float cbin[4], sprod;
  int i, j;
  ...
  cbin[0] = 2 * cang - 1;
  for (i = 0; i < event_hits->ntot(); i++) {
    sprod = dir[3 * i] * direct[0] + dir[3 * i + 1] * direct[1] +
            dir[3 * i + 2] * direct[2];
...
inline void timefit::lcor(float &alpha, float &t0, float &tout, int *bin,
                          float *cor) {
  float costh[5];
  float time, t0bin, dt;
  ...
  for (i = 0; i < 5; i++)
    if (count[i] > 0) {
      ...
      for (max = 100 * i, j = 100 * i + 1; j < 100 * (i + 1); j++)
        if (time_histo[j] > time_histo[max])
          max = j;
...
inline void timefit::maketof(float alpha, float t0, float tout, float *origtof,
                             int *bin, float *cor) {
  int i;
...
inline float timefit::addloglik(float *vertex, double *direct, double cang,
                                float ll0) {
...
// fit the vertex time and direction
// **********************************************
float timefit::fittime_lcor(short int fast, float *vertex, float *direct,
                            float &dt) {
  int maxprod;
  double cen[3], cang;
...
// fit the vertex time and direction
// **********************************************
float timefit::chargetest(float *vertex, float *direct, float &qin,
                          float &qoutplus, float *qoutminus) {
...
float timefit::goodness(float &ll, float *vertex, float *gdnset) {
  ...
  for (denall = gdn = 0, set = offs = ind = 0; set < (int)nlike;
       offs = offset[set++]) {
    if (bgfit)
      denom = 1;
    else
...
int timefit::nwind(float *vertex, float tmin, float tmax, float *ttof,
                   int *cab) {
  int ind, nw;
  float dt;
...
int timefit::nwind(float *vertex, float tmin, float tmax, float *ttof, float *d,
                   int *cab) {
  int ind, nw;
  float dt;
