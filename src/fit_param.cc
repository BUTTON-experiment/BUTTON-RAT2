...
char fit_param::parse(char *line, unsigned char length, char *key1, char *key2,
  ...
  if (key_length1 > 0) {
    for (pos1 = 0; pos1 < length - key_length1; pos1++)
      if (!strncmp(line + pos1, key1, key_length1))
        break;
    pos1 += key_length1;
    if (pos1 >= length)
      return 0; // return, if not found
  } else
    pos1 = 0;
  // check key2, if not an empty string
  if (key_length2 > 0) {
    for (pos2 = 0; pos2 < length - key_length2; pos2++)
      if (!strncmp(line + pos2, key2, key_length2))
        break;
    pos2 += key_length2;
    if (pos2 >= length)
      return 0; // return, if not found
  ...
  if (key_length3 > 0) {
    for (pos3 = 0; pos3 < length - key_length3; pos3++)
      if (!strncmp(line + pos3, key3, key_length3))
        break;
    pos3 += key_length3;
    if (pos3 >= length)
      return 0; // return, if not found
...
inline void fit_param::parse2(char *line) {
  ...
  if (parse(line, length, (char *)"PMT", (char *)"ime", (char *)"esolution",
            FLOAT_TYPE, &tres))
    return;
  if (parse(line, length, (char *)"PMT", (char *)"oincidence", (char *)"ime",
            FLOAT_TYPE, &tcoin))
    return;
  if (parse(line, length, (char *)"air", (char *)"istance", (char *)"raction",
            FLOAT_TYPE, &dlim))
    return;
  if (parse(line, length, (char *)"air", (char *)"ime", (char *)"raction",
            FLOAT_TYPE, &tlim))
    return;
  if (parse(line, length, (char *)"aximum", (char *)"its", (char *)"ombin",
            SHORT_INT_TYPE, &nselall))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"rid", (char *)"onstant",
            FLOAT_TYPE, &gcon0))
    return;

  if (parse(line, length, (char *)"lusfit", (char *)"rid", (char *)"onstant",
            ARRAY_FLOAT_TYPE, &cgrid))
    return;
  if (parse(line, length, (char *)"all", (char *)"istance", (char *)"ertices",
            FLOAT_TYPE, &dw4hit))
    return;
  if (parse(line, length, (char *)"all", (char *)"lusfit", (char *)"ertex",
            FLOAT_TYPE, &dwfit))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit", (char *)"pening",
            FLOAT_TYPE, &clusgtheta0))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit",
            (char *)"ositive", FLOAT_TYPE, &clusgthetaplus))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit",
            (char *)"egative", FLOAT_TYPE, &clusgthetaminus))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit", (char *)"eight",
            FLOAT_TYPE, &clusgdirweight))
    return;
  if ((np == -1) && (parse(line, length, (char *)"umber", (char *)"lusfit",
                           (char *)"asses", SHORT_INT_TYPE, &np))) {
    if (np < 1) {
      np = -1;
      return;
    }
  ...
  if (parse(line, length, (char *)"inal", (char *)"oodness",
            (char *)"ifference", FLOAT_TYPE, &lastdiff))
    return;
  if ((np != -1) && (parse(line, length, (char *)"all", (char *)"nvoke",
                           (char *)"iner", ARRAY_FLOAT_TYPE, cdwall)))
    return;
  if ((np != -1) && (parse(line, length, (char *)"oodness", (char *)"ifference",
                           (char *)"kim", ARRAY_FLOAT_TYPE, gdiff)))
    return;
  if (parse(line, length, (char *)"inal", (char *)"oodness", (char *)"raction",
            FLOAT_TYPE, &lastfrac))
    return;
  if ((np != -1) && (parse(line, length, (char *)"oodness", (char *)"kim",
                           (char *)"raction", ARRAY_FLOAT_TYPE, gfrac)))
    return;
  if (parse(line, length, (char *)"rid", (char *)"ime", (char *)"indow",
            FLOAT_TYPE, &tim0))
    return;
  if ((np != -1) && (parse(line, length, (char *)"oodness", (char *)"ime",
                           (char *)"indow", ARRAY_FLOAT_TYPE, time)))
    return;
  if ((np != -1) && (parse(line, length, (char *)"lusfit", (char *)"inimum",
                           (char *)"adius", ARRAY_FLOAT_TYPE, rmin)))
    return;
  if ((np != -1) && (parse(line, length, (char *)"lusfit", (char *)"top",
                           (char *)"adius", ARRAY_FLOAT_TYPE, rstop)))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"pening", (char *)"ngle",
            ARRAY_FLOAT_TYPE, clustheta0))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"ositive", (char *)"ngle",
            ARRAY_FLOAT_TYPE, clusthetaplus))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"egative", (char *)"ngle",
            ARRAY_FLOAT_TYPE, clusthetaminus))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"ngle", (char *)"eight",
            ARRAY_FLOAT_TYPE, clusdirweight))
    return;
  if (parse(line, length, (char *)"BONSAI", (char *)"rid", (char *)"onstant",
            FLOAT_TYPE, &bgrid))
    return;
  if (parse(line, length, (char *)"all", (char *)"BONSAI", (char *)"ertex",
            FLOAT_TYPE, &bdwfit))
    return;
  if (parse(line, length, (char *)"all", (char *)"nvoke", (char *)"ine",
            FLOAT_TYPE, &bdwall))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"pening", (char *)"ngle",
            FLOAT_TYPE, &bc0))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"ngle", (char *)"ositive",
            FLOAT_TYPE, &bpangle))
    return;
  if (parse(line, length, (char *)"nitial", (char *)"ngle", (char *)"egative",
            FLOAT_TYPE, &bmangle))
    return;
  if (parse(line, length, (char *)"inal", (char *)"ikelihood",
            (char *)"ifference", FLOAT_TYPE, &blastdiff))
    return;
  if (parse(line, length, (char *)"inal", (char *)"ikelihood",
            (char *)"raction", FLOAT_TYPE, &blastfrac))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"pening", (char *)"ngle",
            FLOAT_TYPE, &oc0))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"ngle", (char *)"ositive",
            FLOAT_TYPE, &opangle))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"ngle", (char *)"egative",
            FLOAT_TYPE, &omangle))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"ikelihood", (char *)"iff",
            FLOAT_TYPE, &obgdiff))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"ikelihood",
            (char *)"raction", FLOAT_TYPE, &obgfrac))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"tart", (char *)"adius",
            FLOAT_TYPE, &obrmin))
    return;
  if (parse(line, length, (char *)"oarse", (char *)"top", (char *)"adius",
            FLOAT_TYPE, &obrstop))
    return;
  if (parse(line, length, (char *)"ine", (char *)"pening", (char *)"ngle",
            FLOAT_TYPE, &cc0))
    return;
  if (parse(line, length, (char *)"ine", (char *)"ngle", (char *)"ositive",
            FLOAT_TYPE, &cpangle))
    return;
  if (parse(line, length, (char *)"ine", (char *)"ngle", (char *)"egative",
            FLOAT_TYPE, &cmangle))
    return;
  if (parse(line, length, (char *)"ine", (char *)"ikelihood", (char *)"iff",
            FLOAT_TYPE, &cbgdiff))
    return;
  if (parse(line, length, (char *)"ine", (char *)"ikelihood", (char *)"raction",
            FLOAT_TYPE, &cbgfrac))
    return;
  if (parse(line, length, (char *)"ine", (char *)"inimum", (char *)"adius",
            FLOAT_TYPE, &cbrmin))
    return;
  if (parse(line, length, (char *)"ine", (char *)"top", (char *)"adius",
            FLOAT_TYPE, &cbrstop))
    return;
}
...
fit_param::fit_param(void) {
  ...
  std::string path = getenv("BUTTONDATA");
  ...
  if (pfile != 0) {
    ...
    printf(
        "Could not find local fit_param.dat file, loading in default file.\n");
  ...
  for (ar = 0; ar < np; ar++) {
    ...
    if (clustheta0[ar] == -1)
      exit(1);
    if (clusthetaplus[ar] == -1)
      exit(1);
    if (clusthetaminus[ar] == -1)
      exit(1);
