...
#include <G4VSolid.hh>
#include <GeoButtonShieldFactory.hh>
#include <RAT/DB.hh>
#include <RAT/Log.hh>
...
G4VPhysicalVolume *GeoButtonShieldFactory::Construct(RAT::DBLinkPtr table) {
  ...
  RAT::DBLinkPtr pmtinfo =
      RAT::DB::Get()->GetLink(table->GetS("pmtinfo_table"));
  ...
  G4VisAttributes *innerVis =
      GetVisAttributes(RAT::DB::Get()->GetLink(table->GetS("inner_back_vis")));
  G4VisAttributes *vetoVis =
      GetVisAttributes(RAT::DB::Get()->GetLink(table->GetS("veto_back_vis")));
  ...
  try {
    ...
    if (inner_orient_str == "manual")
      ...
      RAT::Log::Die("GeoBuilder error: Unknown inner PMT orientation " +
                    inner_orient_str);
  ...
  try {
    ...
    if (veto_orient_str == "manual")
      ...
      RAT::Log::Die("GeoBuilder error: Unknown veto PMT orientation " +
                    veto_orient_str);
  } catch (RAT::DBNotFoundError &e) {
  }
  ...
  for (int i = innerStart; i < (innerStart + innerLen); i++) {
    ...
    if (inner_orient_manual)
      ...
      if (orient_point_array.size() != 3)
        RAT::Log::Die(
            "GeoBuilder error: orient_point_inner must have 3 values");
    ...
    new G4LogicalBorderSurface(volumeName + "_back_" + std::to_string(i) +
                                   "_border",
  ...
  for (int i = vetoStart; i < (vetoStart + vetoLen); i++) {
    ...
    if (veto_orient_manual)
      ...
      if (orient_point_array.size() != 3)
        RAT::Log::Die("GeoBuilder error: orient_point_veto must have 3 values");
      orient_point.set(orient_point_array[0], orient_point_array[1],
                       orient_point_array[2]);
      orient = (orient_point - position).unit();
    }
...
G4OpticalSurface *GeoButtonShieldFactory::GetSurface(std::string surface_name) {
  if (RAT::Materials::optical_surface.count(surface_name) == 0)
    RAT::Log::Die("error: surface " + surface_name + " does not exist");
  return RAT::Materials::optical_surface[surface_name];
}

G4VisAttributes *
GeoButtonShieldFactory::GetVisAttributes(RAT::DBLinkPtr table) {
  try {
  ...
  try {
    ...
    if (color.size() == 3) // RGB
      ...
      RAT::warn << "error: " << table->GetName() << "[" << table->GetIndex()
