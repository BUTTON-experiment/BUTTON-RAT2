...
/// Updated by A. Tarrant 2025 for rat2
// no longer need to physically input results updated this to happen automatically
// still make sure the bins are the same between files
...
void timeResiduals(const char *file, const char *file1) {
    
    Double_t light_vel = 21.8;// 21.8 for water, 20. for wbls/ls [cm/ns] velocity was calulated for gd-water, and 20x20m tank size, might need adjustment
 
    TH1D *timeRes = new TH1D("timeRes","time residuals",400,-130,30); // 0.4 ns bins per bin  
    ...
    for (int i = 0; i < T->GetEntries(); i++) {
	    ...
	    for (int j = 0; j<ev->GetPMTCount();j++) {
                ...
                p_t = pmt->GetTime() + t_t;
//                printf("%f %f\n",v_t,p_t);
                if(typ[pmt->GetID()]==1)  timeRes->Fill(-(p_t-v_t));//-(p_t-v_t)); // only ID PMTs
                
            }
    

        //// stolen code from fitquad
/*
...
//                printf("%f %f\n",v_t,p_t);
         ...
         for (int pmtid : inputHandler.GetAllHitPMTIDs()) {
            ...
            t_t = inputHandler.GetTime(pmtid);
            if (time > 1e6) {
                 continue;
            }
            p_x=pmtposX();
            ...
            p_t = pmt->GetTime() + t_t;
//              printf("%f %f\n",v_t,p_t);
            timeRes->Fill(-(p_t-v_t));//-(p_t-v_t)); // only ID PMTs
    ...
    myfile.close();
    timeRes->SaveAs(file1);//"timeRes.root");
