...
type: "tube",
r_max: {rPMT+tBSHEET+tBSHEETOffset}, // changed to accommodate 0.5m-thick layer of concrete on walls (L. Kneale)
size_z: {zPMT+tBSHEET+tBSHEETOffset}, // changed to put tarp behind washer plates
position: [0.0, 0.0, 0.0],
...
mother: "black_sheet",
type: "tube",
r_max: {rPMT+tBSHEETOffset}, // changed to put tarp behind washer plates
size_z: {zPMT+tBSHEETOffset},
position: [0.0, 0.0, 0.0],
material: "doped_water",
color: [0.2,0.2,0.9,0.2],
drawstyle: "solid"
...
photocoverage=photocoverage, pmtRad = pmtRad):
    ...
    encapsulationCSV = open('encapsulationCSV.csv','w')
...
delta = 250.,tolerance = 200., photocoverage=0.205, pmtRad = 126.5, frameRad = 1660.):
    ...
    moduleHeight = arr.array('d', [625.,-625.])
...
tBSHEET = tBSHEET, tBSHEETOffset= tBSHEETOffset, dTANK = dTANK,tTANK = tTANK,dAIR = dAIR ,dCONC = dCONC,tCONC = tCONC, dROCK = dROCK, pmtCnt = cnt )
...
topnBase = arr.array('d', [1250.,-1250.])
...
moduleHeight = arr.array('d', [540.,-540.])
#HERE
for j in range(nRings):    
    for i in range(sides):
        ...
        _geoFile +=  f"position: [{Ux}, {Uy}, {Uz+(-(verL + crossSec) /2.)}],\n"          
        #_geoFile +=  "position: [0.0, 0.0, 0.0],\n" 
        _geoFile +=  f"rotation: [{DirUx}, {DirUy}, {DirUz}],\n"          
        _geoFile +=  "material: \"stainless_steel\"\n"
        _geoFile +=  "drawstyle: \"wireframe\",\n"
...
data = pd.read_csv("encapsulationCSV.csv")
...
geofile = open(f"button_frame.geo","w+")
geofile.writelines(_geoFile)
geofile.close


frameUfile = open(f"FRAMEUPRIGHTSINFO.ratdb","w+")
frameUfile.writelines(_frameUinfo)
frameUfile.close


frameMfile = open(f"FRAMEMODULESSINFO.ratdb","w+")
